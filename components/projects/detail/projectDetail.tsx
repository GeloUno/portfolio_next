...
function DetailProject({
  project: { title, description, technology, github, image, web },
}: IDetailProjectProps) {
  return (
    <div
    ...
    >
      ...
      <div data-testid="DetailProjectDescription" className=" px-6">
        {Array.isArray(description) ? (
          description.map((des) => (
            <h3 key={des} className={`pb-2`}>
              {des}
            </h3>
          ))
        ) : (
      ...
      >
        <div className="w-full text-center font-bold">technologies used:</div>
        {technology.map((tech) => {
          return (
            <div
              data-testid="DetailProjectTechnology"
              key={tech}
              className="flex flex-col w-20 h-20 p-3 m-3 items-center"
            >
              <div
              ...
              <h3 data-testid="DetailProjectImageDescription">{tech}</h3>
            </div>
          );
        })}
      </div>
      {(github || web) && (
        <div className="w-full text-start  text-center font-bold  pb-3">
          {github && web ? `links:` : `link:`}
        </div>
      )}
      <div
        ...
        {github && (
          <Link href={``}>
            <a
              ...
              href={`${github}`}
            >
              <div className="h-10 w-10 cursor-pointer hover:scale-110">
                <Image
