...
function FirstFourTechnologyImage({ technology }: IFirstFourTechnologyImageProps) {
    ...
    return (
        <div
            data-testid="firstFourTechnology"
            className='flex flex-row justify-around self-auto w-full'>

            {firstfourTech.map(tech => {
                return (
                    <div
                        data-testid="firstFourTechnologyBody"
                        className='flex p-1 w-10 h-10 sm:w-16 sm:h-16' key={`logo of ${tech}`}>
                        <Image
                            src={`/images/technology/${tech}.svg`}
                            width={40}
                            height={40}
                            alt={`technology image ${tech}`}
                            data-testid="firstFourTechnologyImage"
                        />
                    </div>
                )
            })}
